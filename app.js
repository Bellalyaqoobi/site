...
function calculateAndShowPrice(distance) {
    ...
    if (distanceElement) {
        distanceElement.textContent = `${distance.toFixed(1)} کیلومتر`;
    }
}
    ...
    setTimeout(() => {
        ...
        if (suggestionsList && popularDestinations.length === 0) {
            ...
            defaultSuggestions.forEach(destination => {
                ...
                suggestionItem.setAttribute('data-destination', destination);
...
// تابع‌های اضافی برای پنل ادمین
async function loadDrivers() {
    // بارگذاری لیست رانندگان
}

async function loadAdminTrips() {
    // بارگذاری سفرهای ادمین
}

async function loadAdminDiscounts() {
    // بارگذاری تخفیف‌های ادمین
}

async function loadAdminSupport() {
    // بارگذاری درخواست‌های پشتیبانی
}

// تابع برای دکمه‌های عملیاتی در جداول
window.viewTripDetails = function(tripId) {
    showNotification('جزئیات سفر نمایش داده شد', 'info');
};

window.editUser = function(userId) {
    showNotification('ویرایش کاربر', 'info');
};
